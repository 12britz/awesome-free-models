...
- [onomeo](https://onomeo.com/) — **47 models, daily check-in credits, public beta** — OpenAI-compatible API and web chat behind one key. Credits come from a daily check-in (50,000 on day 1, up to 200,000/day on a 7-day streak); most models run on upstream providers’ free tiers. 13 higher-cost models (Claude, GPT-6, Gemini Pro and others) are capped at 50,000 credits/day unless you buy credits ($5 = 1M). No credit card. Some upstream free tiers may train on prompts; the model list shows each provider’s policy. Public beta: not every feature is guaranteed to work, and feedback is welcome. **One-person project; listed by the operator.**
